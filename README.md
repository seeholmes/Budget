...
Household names and income are edited directly in the Papa and Mama Dashboard cards. The Pay tab stores one shared biweekly payday and a separate net paycheck for each person, then derives average monthly income automatically. A recurring actual transfer amount and direction can be entered for every payday. Each pay period counts that transfer as incoming for the receiver and outgoing for the sender. The Dashboard keeps its calculated monthly transfer as guidance and shows its equivalent every-payday amount using `monthly × 12 ÷ 26`.
