...
The Cash Flow tab has remembered Papa/Mama and 3-month/6-month/1-year selectors, and shows one personal ledger at a time. Pay periods are grouped by payday month, with three-paycheck months highlighted. Each month heading reconciles incoming, outgoing, and net by summing the pay-period ledgers shown beneath it, so a 14-day period that crosses a calendar boundary still balances with its parent month. Month headings can collapse every paycheck in that month while keeping the reconciliation visible, and each visible pay period can still be collapsed independently. The toolbar's Expand all and Collapse all actions control both levels. The current period opens by default and future pay periods stay collapsed. Expanded desktop ledgers use a transaction table with a running balance; mobile keeps the compact stacked transaction view.

Scheduled spending is managed as Weekly Payments, Monthly Payments, and Annual Payments on the Spending tab. Subscriptions are managed separately on the Subscriptions tab, and every item is assigned to the person responsible for paying it. Payments and subscriptions can be excluded from household transfer guidance as personal spending while remaining visible in outgoing totals and Cash Flow ledgers. Weekly items use the actual weekly payment and a weekday, then appear on that day in every pay-period ledger; monthly and annual equivalents are derived automatically. Annual items use the actual payment amount and appear in the responsible person's ledger and the annual look-ahead. The header includes a saved light/dark mode toggle.
